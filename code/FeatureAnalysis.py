...
def mll_fa_process(args, feature_vectors, labels, after_ps=False, ind=False):
    ...
    if not issparse(feature_vectors):
        return fa_process(args, feature_vectors, labels.toarray(), after_ps, ind)
    else:
        ds = True if issparse(feature_vectors) else False  # data sparse
        out = fa_process(args, feature_vectors.toarray(), labels.toarray(), after_ps, ind)
        return lil_matrix(out) if ds else out
